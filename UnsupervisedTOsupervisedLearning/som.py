...
###This is the UNSUPERVISED part of our model/ learning as we only use a training data and we do not have a test data ###
...
### UNSUPERVISED LEARNING ends here###

print("\n\n\n Let us start our supervised learning")


#create our training data
customers = df.iloc[:, 1:].values #we create a dataset consisting of all features accept the customer ID

#Let us standard scale the customer data
print("Let us standard scale the customer data")
ss = StandardScaler()
customers = ss.fit_transform(customers)

#create a numpy array of zeros to hold accept/reject of credt info
fraud_info = np.zeros((len(customers), 1))

#If customerID is present in frauds, then our fraud_info will be 1 for that customer
print("If customerID is present in frauds, then our fraud_info will be 1 for that customer")
for i in range(len(df)):
	if df.iloc[i,0] in frauds:
		fraud_info[i] = 1 

#Let us create an ANN to train our customer data against their corresponding fraud info 
print("Let us create an ANN to train our customer data against their corresponding fraud info")
model = Sequential()

#Add the input layer and first hidden layer
model.add(Dense(units=2, activation='relu', input_dim=15, init='uniform'))

#Add the output layer
model.add(Dense(units=1, activation='sigmoid', init='uniform'))

#Let us compile the ANN
model.compile(loss='binary_crossentropy', metrics=['accuracy'], optimizer='adam')

#Let us fit our model by fitting the customers' info with their corresponding fraud info
model.fit(customers, fraud_info, batch_size=1, epochs=3) #Let us keep 3 epochs and batch_size=1 as the dataset is small

y_pred = model.predict(customers) #This will give the probabiity of fraud being committed by the customer

#Let us concatenate the fraud_info numpy array to our customers dataset.
#We want concatenation to the right of the dataset along the column and hence axis=1
customer_fraud_probabilities = np.concatenate((df.iloc[:,0:1].values, y_pred), axis=1) 
print("The customer ID and the probabiity that they are frauds are")
print(customer_fraud_probabilities)

print("The customer ID and the probabiity that they are frauds sorted in the ascending order of probability is ")
customer_fraud_probabilities = customer_fraud_probabilities[customer_fraud_probabilities[:, 1].argsort()]
print(customer_fraud_probabilities)